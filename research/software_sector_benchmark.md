...
**这份报告在解决什么问题?** AI 来了之后,软件行业要么变天、要么大涨——**约 50 家上市软件公司,谁会赢、谁会被替代?**

**用大白话讲清楚 AI 对软件行业的冲击:**
- **传统软件按"座席收费"**——比如 Salesforce 一个销售员一年付几千美元。
- **AI agent 会替代座席**——一个 AI 顶 5 个人。**人少了,座席收费就缩水了**——这就是软件行业的危机。
- **但不是所有软件都受冲击。** 三类幸存者:
  1. **按用量收费的**(如云数据库 Snowflake)——AI 用得越多、它收得越多;
  2. **拥有"agent 必须经过它"的数据基底**(如微软 Azure、ServiceNow 工作流)——AI 离不开它们;
  3. **专有数据/受监管行业**(如 Veeva 服务药企)——AI 学不来、也拿不走。

**关键判别指标:NRR(净收入留存率)**
- 这是一个数:**老客户今年比去年多付了百分之多少。**
- >110% 且在升 = 老客户每年加买,健康(微软、ServiceNow);
- <100% = 老客户在砍预算,基盘在缩(Asana 95%、ZoomInfo 90%)——**这是 AI 替代的铁证**。

**赢家/输家 5 层分级(本报告核心):**
- 🟢 **核心赢家:** 微软、谷歌、亚马逊 AWS、Datadog、CrowdStrike、ServiceNow、Atlassian、Figma、Shopify、Veeva,以及私有的 Databricks。
- 🟢* **质地顶级但价格太贵:** Palantir(前瞻 PE 约 97×)——质量满分,但价格透支。
- 🔵 **稳健受益:** Palo Alto、Snowflake、SAP、HubSpot、Twilio 等。
- 🟡 **硬币(看转型成败):** Salesforce(押 AI agent 转型)、Adobe(被 AI 商品化威胁)、Workday(按人头收费=最暴露)。
- 🟠 **承压融冰:** GitLab、Asana、Zoom、Dropbox、Gartner。
- 🔴 **被颠覆/输家:** Chegg(收入 -48%)、ZoomInfo(指引砍到 -4%)、C3.ai(收入 -46%)——财报已经在流血。

**文中频繁出现的公司速查:**
- **微软(MSFT):** 唯一全栈赢家(模型+云+应用+安全+身份);
- **谷歌(GOOGL):** 唯一公开市场能买到的"模型层"敞口(Gemini+TPU+搜索);
- **亚马逊(AMZN):** AWS 是利润引擎,自研 Trainium 芯片;
- **Snowflake(SNOW):** 云数据仓库(企业数据库);
- **Databricks(私):** Snowflake 头号对手,2H26 IPO;
- **Datadog(DDOG):** 云端监控(AI 用得多越要监控);
- **CrowdStrike(CRWD):** 云端安全(每个 AI agent 都是新攻击面);
- **ServiceNow(NOW):** 企业工作流自动化(IT/HR);
- **Atlassian(TEAM):** 开发协作工具(Jira/Confluence);
- **Salesforce(CRM):** CRM 龙头,正激进推 Agentforce(AI agent);
- **Adobe(ADBE):** 创意软件(PS),被 AI 生成图威胁但在 Firefly 反击;
- **Veeva(VEEV):** 药企专用 CRM(在抢 Salesforce 份额);
- **Shopify(SHOP):** 电商建站(按交易额抽成,跟座席无关);
- **Palantir(PLTR):** AI 数据/本体平台,政府+商业;
- **ZoomInfo(ZI/GTM):** B2B 销售情报数据,被 AI 直接商品化;
- **Chegg(CHGG):** 在线作业辅导,被免费 LLM 抹掉。

**关键名词:**
- **NRR(净收入留存率):** 老客户今年比去年多付的比例,>110% 健康、<100% 危险;
- **GRR(毛留存):** 剥掉扩张,只看老客户流失;
- **ARR(年化经常性收入):** 订阅类公司的核心指标(月收入 × 12);
- **agent / agentic AI:** 能自主完成多步任务的 AI;
- **座席定价 / 消耗定价 / 结果定价:** 按人头 / 按用量 / 按成果收费;
- **SoR(系统-of-record):** 企业核心数据系统(CRM/ERP/HR);
- **SaaS:** 软件即服务,订阅模式;
- **ARR ratchet:** 每年订阅金额自动涨价的机制。

**你应该带走的 3 个 takeaway:**
1. **AI 不是一刀切杀软件——而是杀按人头收费、卖通用数据的;** 救拥有数据基底/按用量收费的。
2. **NRR 是"AI 是否已开始杀这家公司"的最灵敏指标。** 跌穿 100% 就是危险信号。
3. **核心赢家(微软/谷歌/AWS/CrowdStrike/ServiceNow/Datadog/Palantir)是公开市场最确定的 AI 软件长持名单。**

**重要诚实警告:**
- "市场份额"多为研究机构估算,按方向看;
- 私有公司(Databricks/Cursor/Sierra)的 ARR 是创始人/VC 口径,**未经审计、打折看**;
- 估值/数字 5 月口径,**下单前需在彭博等终端复核**。
