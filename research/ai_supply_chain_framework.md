...
## 📖 五分钟读懂(中文导读,给完全没接触过 AI 供应链投资的读者)

**这份是整个研究项目的"地基文档"(英文)——后面所有的深度报告都建在这上面。**

**它在解决什么问题?**
- 不是"AI 是不是真趋势"(是,这是共识);
- 而是"AI 链上哪些环节的估值已经被市场算对了、哪些没算对、哪里能赚钱"。

**它提出的方法论(三句话):**
1. **每个环节都有"老 TAM(总市场)"和"新 TAM"两个端点**——估值的关键不在某一个数,在两端之间的"桥"(volume × content × ASP × attach)。
2. **反向"已定价"测试:** 从当前股价反推"市场暗含的市占率和 TAM",看它是不是已经透支。
3. **公司天花板尺寸表:** 把每家公司"在最理想情况下能涨到多大"算出来,跟今天的市值比。

**核心概念:**
- **chokepoint(卡脖子):** 供应链上不可替代、有定价权的环节;
- **content per unit(每单位含量):** 每颗 GPU/芯片里某零件的含量,是弹性的核心变量(2026 年 AI capex +77%,芯片数量只 +25%,差额都在"含量");
- **investability gap(可投资性缺口):** 最硬的卡脖子常常买不到/买得贵——这是反复出现的主题;
- **§6b 写作标准:** 给非技术 IC 读者写作的纪律,后面所有 deepdive 都遵守。

**核心五维评分(后面 memo 都在用):**
- E 弹性(对 AI 的杠杆)、D 护城河耐久(到 2030 不被替代)、B 基数空间(小基数更好)、V 估值空间(便宜更好)、H 可长持(周期/二元风险低)。

**英文的原因:** 这是最早写的文档,作为"工作底稿"留存。后续所有中文 memo(`optical/memory/inp/power/software/synthesis/ten_year` 等)都已遵循它的方法论,**所以非技术读者可以跳过本文,直接读 `READ_FIRST.md`,再按推荐顺序看具体 deepdive。**

**真要读懂这份英文的:** 重点看 §0-§3(问题陈述、方法论、卡脖子地图)、§5(假设银行,逐个 hypothesis)和 §6b(写作标准)。§4 是早期的"价格 vs 现实"对比,部分数字已被后续 memo 更新覆盖。
