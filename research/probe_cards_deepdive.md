...
## 📖 五分钟读懂(给完全没接触过半导体测试的读者)

**探针卡是什么?(三句话讲清楚)**
1. **芯片造完,出厂前要"体检"**——把每颗芯片接到测试机,看它能不能正常工作。
2. **探针卡(probe card)就是测试机和晶圆之间的"针床"**——用成千上万根极细的针,扎到芯片的焊盘上传电信号,做这个体检。
3. **每代芯片(2nm、HBM、3D 封装)针的数量和密度都翻倍,且探针卡是"消耗品"**(每万次扎完就要换),这是它"单向棘轮"的来源。

**为什么是 AI 卡脖子?**
- **AI 芯片测试时间是消费芯片的 3-5 倍**(更多接口、更复杂);
- **HBM 每多一层、就多一轮测试**(HBM3→HBM4 从 12 层到 16 层,测试次数线性涨);
- 探针卡是消耗品,**收入随测试次数复利**——不是一次性资本品。

**两家上市公司:**
- **FormFactor(FORM,美股):** 全球第一,DRAM/HBM 探针卡龙头;
- **Technoprobe(TPRO.MI,意大利米兰上市):** 全球第二,SoC/逻辑芯片探针卡龙头。

**本报告核心结论:**
- **对的卡脖子,但增速最慢**(我们后来意识到):约 10% 增速(整体),AI 切片约 20-30%——不是 HBM/光通讯那种 40%+ 的弹性;
- **估值已满**(FormFactor、Technoprobe 都不便宜);
- **后续 deepdive 用 Advantest(测试机龙头)升级了这一逻辑**——Advantest 是同样测试棘轮、但增速和 AI 弹性都更高(见 `next_chokepoint_hunt.md`)。

**用大白话比喻:** 探针卡是"卖芯片体检用的针床"——客户买芯片越多、HBM 越叠越高、探针卡卖得越多。但增速温和,适合稳健长持,不是高弹性。

**关键技术名词:**
- **probe card / 探针卡:** 测试针床;
- **ATE(自动测试设备):** 整个测试机,Advantest/Teradyne 卖;
- **wafer / 晶圆:** 还没切割的芯片晶圆;
- **WLT(晶圆级测试) / FT(成品测试):** 测试的两个阶段;
- **HBM stack 测试:** 多层堆叠后逐层测试,工艺难;
- **MEMS / lithographic probes:** 探针的两种工艺,FormFactor 是 MEMS 龙头;
- **消耗品复利:** 收入随客户用量(而非客户数量)增长。

**你应该带走的 3 个 takeaway:**
1. **探针卡是真卡脖子(消耗品 + 测试强度单向棘轮)、但是慢增速版本**,FormFactor / Technoprobe 适合稳健仓位。
2. **同样逻辑、更高弹性的升级版是 Advantest**(测试机龙头)——见 `next_chokepoint_hunt.md` 和 `ten_year_chokepoint_conviction.md`。
3. **测试整体(Advantest + KLA + FormFactor + Technoprobe)是十年 conviction 的"卫星仓"——架构无关、单向棘轮、整合免疫力强。**
