...
- **"更难被颠覆"派(Aaron Levie / 垂直 SaaS):** Box 的 Levie**承认座席定价会死,但否认 SaaS 会死**——软件难不在 UI,而在"编码了业务流程、合规、运营现实",这些不会消失;agent 反而**需要**上下文和数据,他预测"agent 数量是人的 100~1000 倍",所以软件要"无头化(headless,可被 agent 调用)"——这杀死按座席、却放大用量。
