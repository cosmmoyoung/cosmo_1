...
**The single most important conclusion:** there is an **"investability gap."** The *truest* monopolies in the AI BOM — Ajinomoto's ABF film (~95%), AGC/Hoya EUV mask blanks (~93%), Resonac's MR-MUF underfill — are **buried inside giant diversified companies where the chokepoint is <5% of group revenue.** Their scarcity rent is real but **does not move the parent stock.** Owning them is *not* how you express the BOM-walk thesis.
...
|---|---|---|:-:|:-:|:-:|:-:|:-:|---|
| 1 | **Probe cards** (HBM/2nm wafer test) | FormFactor `FORM` ~$10.5bn; Technoprobe `TPRO.MI` ~$12bn | 4 | 4 | 5 | 4 | **5** | ★ **TRUE & ownable — top pick of the walk** |
| 2 | **D2W hybrid bonding** (next-gen HBM/logic stacking) | BESI `BESI.AS` (hybrid-bond ~⅓ of co by '26) | 4 | 4 | 4 | 4 | **3** | ★ **TRUE forward chokepoint — owns the *next* node** |
| 3 | **AI back-end materials basket** (MR-MUF + EMC + films) | Resonac `4004.T` ~$19bn (segment >30% sales, +74% profit Q1'26) | 4 | 4 | 4 | 4 | **3** | ★ **Best diversified way to own several at once** |
| 4 | **ABF build-up film** | Ajinomoto `2802.T` ~$23bn (film <5% of group) | 5 | 5 | 5 | 5 | **2** | TRUE archetype, but *buried in a food co* + consensus |
| 5 | **EUV mask blanks** | AGC `5201.T` / Hoya `7741.T` (each <5% of group) | 5 | 5 | 5 | 4 | **1** | TRUE 93% duopoly — *un-investable cleanly* |
| 6 | **HVLP5 copper foil** | Mitsui Kinzoku `5706.T` ~$3.5bn (foil ~10–15%, smelting drag) | 4 | 5 | 4 | 4 | **3** | TRUE at top grade; equity diluted by smelting |
...
| 9 | **EUV pellicle** | Mitsui Chemicals `4183.T` ~$6bn | 5 | 4 | 4 | 4 | **2** | TRUE 70–80% oligopoly — buried in commodity chem |
| 10 | **CoWoS EMC / back-end films** | Sumitomo Bakelite `4203.T`; Resonac | 3 | 3 | 4 | 4 | **3** | Partial → true on specific sub-lines |
| 11 | **224G connectors / NVLink twinax** | Amphenol `APH` ~$100bn+ | 2 | 4 | 5 | 4 | **2** | Real sole-source yield-gate — *but not tiny, mega-cap* |
| 12 | **Vertical power delivery (VPD)** | Vicor `VICR` ~$14bn | 4 | 3 | 5 | 3 | **4** | "Gating *if* it wins Rubin socket" — binary optionality |
| 13 | **Low-loss resin (PPE/BMI)** | Mitsubishi Gas Chem `4182.T` ~$4bn | 3 | 3 | 4 | 4 | **2** | Pricing power via tight supply, *not* monopoly |
| 14 | **Optical micro-optics / isolators** | Tianfu `300394.SZ` ~$38bn | 3 | 3 | 4 | 3 | **2** | Tight (isolators) but proxy is a crowded large-cap |
| 15 | **Test/burn-in sockets** | ISC `095340.KQ`; Leeno `058470.KQ` | 4 | 3 | 4 | 2 | **3** | Small & gating but too fragmented |
| 16 | **HBM TC bonder** | Hanmi `042700.KQ` ~$25bn | 4 | 3↓ | 4 | 3 | **4** | ⚠ **Monopoly BREAKING (71%→20–30% SKH '26)** |
...
- **Why it's the best:** the *only* candidate that clears all five screens, including investability. ~$4–5bn TAM (2026), top-3 ≈ 60% share, MEMS-tip IP moat, bespoke-per-device + consumable (recurring), and **rising structurally with HBM stack KGD test + chiplet/2.5D test intensity.** Technoprobe won ~30% of TSMC 2nm quals.
...
- **Why:** the *forward* gate. As HBM4 → HBM4E and logic move from micro-bump TCB to copper-to-copper hybrid bonding, BESI leads D2W (<10nm placement, next-gen ~50nm in 2026), partnered with Applied Materials. Hybrid-bond revenue €36m (2023) → ~€476m (2026E), ~⅓ of the company — small base, exploding, and a cleaner equity than the diversified materials names.
...
- **Mitsui Kinzoku HVLP5 foil (5706.T):** ~80% of the top grade, sold-out, guidance raised on it — but base-metal smelting dilutes the equity. A "chokepoint with a cyclical anchor strapped to it."
...
| Trap | Why it fails |
|---|---|
| **Hanmi Semiconductor (HBM TC bonder)** | The textbook HBM monopoly (~71%) is **actively breaking in 2026** — ASMPT took ~half of SK Hynix HBM4 TCB sets; Micron going BESI sole-source; Hanmi–Hanwha patent fight; SK Hynix share may fall to 20–30%. **Owning the eroding incumbent is the wrong trade; own the share-gainer (BESI/ASMPT).** A live example of "monopoly decay" risk. |
...
| **Amphenol 224G / NVLink copper** | Genuine sole-source, yield-gated technical chokepoint — but TAM is *not* tiny and the proxy is a ~$100bn+ diversified mega-cap. Plus copper is a **bridge** technology (physics reach-limit pushes scale-up to optics over time). |
